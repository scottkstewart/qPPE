...
class SettingsDlg(QDialog):
    ...
    def __init__(self, parent=None):
        ...
        grid.addWidget(self.handle_ppe, 2, 0, 1, 2)

        # checkbox on whether to send emails if PPE is being handled by the app, visibiliy dependent on handle_ppe
        self.send_emails = QCheckBox("&Send email notifiations?")
        self.send_emails.setVisible(self.handle_ppe.checkState())
        self.handle_ppe.stateChanged.connect(lambda: self.send_emails.setVisible(self.handle_ppe.checkState()))
        ...
        grid.addWidget(self.send_emails, 3, 0, 1, 2)
